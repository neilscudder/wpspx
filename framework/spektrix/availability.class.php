...
{
  $href = home_url('/book-online/'.$performance->id);
  ...
  if($now >= $presale_begins){
    $string.= '<br>Presale now on for ' . '<a href="' . home_url('/support-us/friends/') . '">friends of the New Wolsey</a>';
    $string.= '<a class="btn btn-block btn-danger" href="'.$href.'" style="margin-top:10px;">Buy Presale Tickets</a>';
  } else {
    $string.= '<br>Presale begins ' . date($date_format,$presale_begins) . ' for ' . '<a href="' . home_url('/support-us/friends/') . '">friends of the New Wolsey</a>';
  }
  return $string;
}
