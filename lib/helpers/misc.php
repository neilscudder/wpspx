...
function wpspk_bust_cache( $verbose = false ) {
  $cached_files = WPPSX_PLUGIN_DIR . 'cache/*.txt';

  try {
    array_map('unlink', glob($cached_files));
    if($verbose) {
      echo('<div class="notice notice-success is-dismissible"><p>'.__( 'KaPow! Spektrix cache has been Busted!', 'wpspx' ).'</p></div>');
    }
  } catch (Exception $e) {
    if($verbose){
      echo('<div class="notice notice-success is-dismissible"><p>Oops... '.$e->getMessage().'</p></div>');
    }
  }
}
